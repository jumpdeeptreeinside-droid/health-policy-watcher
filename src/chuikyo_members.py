...
from chuikyo_structurize import BODY_DIRS
...
MEMBER_SIDE = {
    ...
    "宮近清文": "支払側", "松浦満晴": "支払側", "眞田享": "支払側", "眞田亨": "支払側", "末松則子": "支払側",
    ...
    "茂松茂人": "診療側", "黒瀨巌": "診療側", "黒瀬巌": "診療側", "太田圭洋": "診療側", "大杉和司": "診療側",
    ...
    "丸山誠": "支払側",   # 経団連（2007年頃）
    "室谷千英": "公益",   # 神奈川県立保健福祉大学顧問（2007年頃）
    "矢内邦夫": "支払側", "石津寿惠": "公益", "白石小百合": "公益",
    "松浦稔明": "支払側", "染谷絹代": "支払側", "榊原純夫": "支払側", "飯沼雅朗": "診療側",
}

# 名前は非貪欲＝「石牟禮武志専門委員」を（石牟禮武志・専門委員）に正しく分ける。部会長・小委員長も肩書に。
NAME_RE = re.compile(r"([一-鿿々]{1,4}?[一-鿿々ぁ-ゖァ-ヶ]{0,6}?)(専門委員|会長代理|部会長|小委員長|委員長|会長|委員|参考人)")
    ...
    for mtg, d in BODY_DIRS.items():
        for f in glob.glob(os.path.join(d, "*.json")):
            ...
            speakers = [r[0] for r in db.execute(
                "SELECT DISTINCT speaker FROM chuikyo_utt WHERE mtg=? AND kai=? AND kind IN ('委員','参考人')", (mtg, kai))]
            for s in speakers:
                matches = [fn for fn in attendees if fn.startswith(s)]
                if len(matches) == 1:
                    fn = matches[0]
                    side = MEMBER_SIDE.get(fn)
                    db.execute("UPDATE chuikyo_utt SET full_name=?, side=? WHERE mtg=? AND kai=? AND speaker=?",
                               (fn, side, mtg, kai, s))
                    resolved += 1
                elif len(matches) > 1:
                    ambiguous += 1
    # 専門委員（看護・薬剤業界代表等）は1号/2号/公益の枠外＝側は「専門委員」として明示
    db.execute("UPDATE chuikyo_utt SET side='専門委員' WHERE kind='委員' AND role='専門委員'")
    db.commit()
