...
# 4分類（2026-07-06 オペ改訂・木内さん設計）: 記事は常に詳細、音声はこの分類で強弱をつける
CATEGORY_LABELS = {
    "A": "国内・診療",       # 診療報酬・調剤・薬機法・医療提供体制・薬局/DS業界など、国内の診療/薬局実務に直結
    "B": "国内・保健",       # 国内のそれ以外の保健（公衆衛生・感染症・介護・統計・審議会の周辺動向など）
    "C": "国際・日本関連",   # 国際保健のうち日本に影響・関係があるもの（WHO決定の国内影響・日米・規制ハーモナイゼーション等）
...
- "B" = 国内・保健: 日本国内で、それ以外の保健・公衆衛生に関わるもの（感染症、介護、統計、審議会の周辺動向など）
- "C" = 国際・日本関連: 国際的な保健ニュースのうち、日本に影響・関係があるもの
- "D" = 国際・その他: 国際的な保健ニュースのうち、日本にはそれほど関わらないもの（各国の国内事情、地域プログラムなど）
...
def score_articles_with_gemini(articles: List["NewsArticle"]) -> List[dict]:
    """Gemini で記事を4分類（A-D）＋WeeklyReport推奨に一括仕分けする"""
    if not GEMINI_API_KEY:
        logger.warning("GEMINI_API_KEY 未設定のため分類をスキップします")
        return [{"cat": None, "weekly": False}] * len(articles)
    ...
    try:
        ...
        logger.warning("google-genai 未インストールのため分類をスキップします")
        return [{"cat": None, "weekly": False}] * len(articles)
    ...
    try:
        ...
        if len(scores) != len(articles):
            logger.warning(f"分類件数不一致: {len(scores)} vs {len(articles)}")
            scores += [{"cat": None, "weekly": False}] * (len(articles) - len(scores))
        return scores
...
class NotionUploader:
    ...
    def add_article(self, article: NewsArticle, score_info: Optional[dict] = None) -> Optional[str]:
        ...
        try:
            ...
            # 4分類をNotionに記録（自動ステータス変更はしない。音声の強弱に使う）
            cat = (score_info or {}).get("cat")
    ...
    def upload_articles(self, articles: List[NewsArticle]) -> Dict[str, int]:
        ...
        for i, article in enumerate(new_articles):
            ...
            if score_info:
                logger.info(f"  分類: {CATEGORY_LABELS.get(score_info.get('cat'), '未分類')} / WeeklyReport推奨: {score_info.get('weekly')}")
