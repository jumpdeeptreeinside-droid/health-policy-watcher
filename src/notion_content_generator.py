...
- 数字は「半角」、記号は「全角」を使用してください。
- **英語表記を音声原稿に残さないでください（最重要）。** 機関名・薬剤名・固有名詞・略語はすべて日本語またはカタカナに直します。例：WHO→世界保健機関、AMR→薬剤耐性、RUXOLITINIB→ルキソリチニブ、Medical Product Alert→医薬品アラート。どうしても略語で読ませたい場合のみカタカナで一字読み（例：ジーディーピー）にしてください。
...
- **改行:** 句点（。）ごとに改行
- **英語を残さない（最重要）:** 機関名・薬剤名・固有名詞・略語はすべて日本語かカタカナに（WHO→世界保健機関、RUXOLITINIB→ルキソリチニブ 等）。アルファベットのまま音声原稿に残さないでください。
